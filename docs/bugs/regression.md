...
| 领域 | 风险 | 推荐回归 |
|---|---|---|
| 多用户隔离 | 新接口遗漏 user_id 过滤导致跨用户数据暴露 | `tests/api/test_user_scope_isolation.py`, 相关 router 单测 |
| 视频软删除 | 查询、搜索、推荐仍返回 deleted 视频 | `tests/api/test_video_api.py`, search/recommendation tests |
