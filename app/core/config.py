...
class Settings(BaseSettings):
    ...
    # AI 响应控制：高并发下优先保障可用性与响应时间
    AI_UPSTREAM_MAX_CONCURRENCY: int = 8
    AI_UPSTREAM_ACQUIRE_TIMEOUT_SECONDS: float = 0.05
    AI_NORMAL_TIMEOUT_SECONDS: float = 8.0
    AI_HIGH_TIMEOUT_SECONDS: float = 5.0
    AI_EXTREME_TIMEOUT_SECONDS: float = 3.0
    AI_NORMAL_MAX_TOKENS: int = 1024
    AI_HIGH_MAX_TOKENS: int = 256
    AI_EXTREME_MAX_TOKENS: int = 96
    AI_CIRCUIT_BREAKER_FAILURE_THRESHOLD: int = 2
    AI_CIRCUIT_BREAKER_RECOVERY_SECONDS: float = 20.0
    AI_RESPONSE_CACHE_MAX_ITEMS: int = 256
    AI_RESPONSE_CACHE_TTL_SECONDS: float = 300.0
    AI_ADMISSION_ENABLED: bool = True
    AI_ADMISSION_MAX_ACTIVE_REQUESTS: int = 80
    AI_ADMISSION_MAX_WAITING_REQUESTS: int = 40
    AI_ADMISSION_WAIT_TIMEOUT_SECONDS: float = 0.15
    AI_ADMISSION_PER_USER_ACTIVE_REQUESTS: int = 6
    AI_EVENT_LOOP_LAG_HIGH_SECONDS: float = 0.25
    AI_EVENT_LOOP_LAG_EXTREME_SECONDS: float = 0.75
    AI_REQUEST_HARD_TIMEOUT_SECONDS: float = 10.0
    ...
    YOUTUBE_DOWNLOAD_IMPERSONATE: str = ""
    YOUTUBE_DOWNLOAD_SLEEP_REQUESTS: float = 0.0
    YOUTUBE_DOWNLOAD_RETRIES: int = 10
    YOUTUBE_DOWNLOAD_EXTRACTOR_RETRIES: int = 3
    MOOC_DIRECT_IMPORT_ENABLED: bool = False
